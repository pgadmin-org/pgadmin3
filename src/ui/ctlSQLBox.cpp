...
#define SQL_KEYWORDS wxT("\
        ABORT ADD AGGREGATE ALL ALTER ANALYZE AND AS ASC ASCENDING AUTHORIZATION BEGIN BY \
        CASE CAST CHECK CHECKPOINT CLASS CLOSE CLUSTER COLUMN COMMENT COMMIT CONVERSION CONSTRAINT COPY CREATE CURSOR \
        CASCADE CASE CAST CHECKPOINT CLASS CLOSE CLUSTER COMMENT COMMIT CONVERSION CONSTRAINT COPY CREATE CURSOR \
        DATABASE DEALLOCATE DECLARE DEFAULT DELETE DESC DESCENDING DO DOMAIN DROP EACH ELSE END EXPLAIN \
        FETCH FOR FOREIGN FREEZE FROM FULL FUNCTION GRANT GROUP \
        HAVING ILIKE IMMUTABLE IMPLICIT IN INDEX INHERITS INNER INSERT INSTEAD INTO IS JOIN KEY LANGUAGE LIKE LISTEN LOAD LOCK MOVE NOT NOTHING NOTIFY NULL \
        OFF OIDS ON OPERATOR OPTION OR ORDER OUTER PLAIN PREPARE PRIMARY PROCEDURE REINDEX REFERENCES REPLACE RESET RETURN RETURNS REVOKE ROLLBACK ROW RULE \
        SCHEMA SELECT SET SEQUENCE SESSION SHOW STABLE STATEMENT STORAGE STRICT TABLE THEN TO TRIGGER TRUNCATE TYPE \
        UNION UNLISTEN UPDATE USING VACUUM VALUE VALUES VIEW VOLATILE WHEN WHERE WITH WITHOUT")
