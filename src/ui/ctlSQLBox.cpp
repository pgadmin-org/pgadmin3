...
#define SQL_KEYWORDS wxT("\
        ...
        CASE CAST CHECKPOINT CLASS CLOSE CLUSTER COLUMN COMMENT COMMIT CONVERSION CONSTRAINT COPY CREATE CURSOR \
        ...
        OIDS ON OPERATOR OPTION OR ORDER OUTER PLAIN PREPARE PRIMARY PROCEDURE REINDEX REFERENCES REPLACE RESET RETURN RETURNS REVOKE ROLLBACK ROW RULE \
        SCHEMA SELECT SET SEQUENCE SESSION SHOW STABLE STATEMENT STORAGE STRICT TABLE THEN TO TRIGGER TRUNCATE TYPE \
        UNION UNLISTEN UPDATE USING VACUUM VALUES VIEW VOLATILE WHEN WHERE WITH WITHOUT")
