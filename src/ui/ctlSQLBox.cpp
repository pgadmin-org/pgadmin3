...
const wxString keywords1=
        ...
        wxT("CASE CAST CHECK CHECKPOINT CLASS CLOSE CLUSTER COLUMN COMMENT COMMIT CONVERSION CONSTRAINT COPY CREATE CURSOR ")
        wxT("CASCADE CASE CAST CHECKPOINT CLASS CLOSE CLUSTER COMMENT COMMIT CONVERSION CONSTRAINT COPY CREATE CURSOR ")
...
const wxString keywords2=
        ...
        wxT("OFF OIDS ON OPERATOR OPTION OR ORDER OUTER PLAIN PREPARE PRIMARY PROCEDURE REINDEX REFERENCES RENAME REPLACE RESET RESTART RETURN RETURNS REVOKE ROLLBACK ROW RULE ")
        wxT("SCHEMA SELECT SET SEQUENCE SESSION SHOW STABLE STATEMENT STORAGE STRICT TABLE THEN TO TRIGGER TRUNCATE TYPE ")
        wxT("UNION UNLISTEN UPDATE USER USING VACUUM VALUE VALUES VIEW VOLATILE WHEN WHERE WITH WITHOUT");
