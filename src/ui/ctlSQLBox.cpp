...
#define SQL_KEYWORDS wxT("\
        ...
        HAVING ILIKE IMMUTABLE IN INDEX INSERT INTO IS JOIN KEY LANGUAGE LIKE LISTEN LOAD LOCK MOVE NOT NOTIFY NULL \
        OIDS ON OPERATOR OPTION OR ORDER PREPARE PRIMARY PROCEDURE REINDEX REPLACE RESET RETURN RETURNS REVOKE ROLLBACK ROW RULE \
        SCHEMA SELECT SET SEQUENCE SESSION SHOW STABLE STATEMENT STRICT TABLE THEN TO TRIGGER TRUNCATE TYPE \
        UNLISTEN UPDATE USING VACUUM VALUES VIEW VOLATILE WHEN WHERE WITH WITHOUT")
